...
def update(code_num, cur_code, need_swap, old_page, code_page, memory_page, page_missing, page_size, code_amount):
    # 打印Task执行情况信息
    ui.textEdit.append(str(code_num).ljust(5) + str(cur_code).ljust(6) + ('是' if need_swap else '否').ljust(5) + (
        str(old_page) if old_page != -1 else '-').ljust(6) + (str(code_page) if need_swap else '-').ljust(3))
    ui.textEdit.setReadOnly(True)
    ui.textEdit.moveCursor(QTextCursor.End)

    if need_swap:  # 发生缺页需要换页
    ...
    if (code_num + 1) >= code_amount:  # 任务完成则打印完成信息
        ui.textEdit.append(str(code_amount) + '条指令(0-' + str(code_amount - 1) + ')已全部完成!')
        ui.textEdit.moveCursor(QTextCursor.End)
    ...
    def run(self):
        while True:  # 模拟级循环
            ...
            while True:  # 任务级循环
                ...
                while True:  # 当前步的运行模式检查与等待输入
                    # 处于暂停模式则在这里循环等待
                    p(run_mode_sem)
                    if run_mode == 1:  # 单步执行模式则重新设定为暂停模式后退出循环，执行完下方的模拟后回到这个位置再次等待
                        run_mode = 0
                        v(run_mode_sem)
                        break
                    elif run_mode == 2:  # 连续执行模式则直接退出循环，执行完下方模拟回到这里后再次直接退出循环
                        v(run_mode_sem)
                        break
                    v(run_mode_sem)

                    p(reset_flag_sem)
                    # 当处于连续执行模式时，会直接退出循环，则连续执行模式下可以由任务级循环的头部来检查是否设定了重置
                    # 单步执行和暂停模式下会在本级循环等待，所以需要单独处理重置
                    if reset_flag:
                        v(reset_flag_sem)
                        tmp_reset_flag = True  # 用于退出两层循环
                        break
                    v(reset_flag_sem)
                ...
                if (code_num + 1) >= code_amount:  # 运行完所有代码
                    p(run_mode_sem)
                    run_mode = 0
                    v(run_mode_sem)
                    p(reset_flag_sem)
